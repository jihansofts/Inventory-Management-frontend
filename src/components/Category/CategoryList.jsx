...
const CategoryList = () => {
  ...
  let Total = useSelector((state) => state.category.TotalList);

  const handlePageClick = async (e) => {
    await CategoryListRequest(e.selected + 1, perPage, searchKeyword);
  };
  const TextSearch = (e) => {
  ...
  const DeleteItem = async (id) => {
    ...
    if (Result.isConfirmed) {
      let ResultDelete = await DeleteCategoryRequest(id);
      if (ResultDelete === true) {
        CategoryListRequest(1, perPage, searchKeyword);
      }
    }
  };
  return (
