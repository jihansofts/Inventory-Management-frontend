...
const ExpenseTypeCreateUpdate = () => {
  ...
  let Total = useSelector((state) => state.expensetype.TotalList);

  const handlePageClick = async (e) => {
    await ExpenseTypeListRequest(e.selected + 1, perPage, searchKeyword);
  };
  const TextSearch = (e) => {
  ...
  const DeleteItem = async (id) => {
    ...
    if (Result.isConfirmed) {
      let ResultDelete = await DeleteExpenseTypeRequest(id);
      if (ResultDelete === true) {
        ExpenseTypeListRequest(1, perPage, searchKeyword);
      }
    }
  };
  return (
